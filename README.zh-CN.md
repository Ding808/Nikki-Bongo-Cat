...
下载 **`Nikki-Bongo-Cat-win-x64.zip`**，完整解压后双击 **`StartPetWithStats.cmd`**。成品包已包含运行组件，**无需安装 .NET、Visual Studio，也无需编译**。GitHub 自动提供的 **Source code** 源码压缩包面向开发者，不包含编译好的统计程序。

每个 Release 还附有 `Nikki-Bongo-Cat-win-x64.zip.sha256` 校验文件。需要核对下载完整性时，在 PowerShell 中运行 `Get-FileHash .\Nikki-Bongo-Cat-win-x64.zip -Algorithm SHA256`，与校验文件中的值比较。
...
| 操作 | 方法 |
| --- | --- |
| 打开统计面板 | 双击桌宠旁的小入口，或在托盘菜单选择“打开统计面板”。 |
| 切换语言 | 点击面板右上角的 **EN**（中文界面显示 **中**），再选择语言；也可使用“高级详情 → 语言”。 |
| 调整统计面板大小 | “高级详情 → 面板大小”，选择预设或“自定义大小…”（65%–175%）。 |
| 移动桌宠 | 解锁后，在可见桌宠本体上按住左键拖动。 |
| 缩放桌宠 | 解锁后，在可见桌宠本体上按住右键拖动。 |
...
“对话次数”对应**去重后读到的用量记录数**，不一定等于聊天窗口或会话线程数量。“模型使用占比”按各模型的令牌数计算，不代表花费占比，也不代表订阅额度的使用比例。界面以整数显示百分比，合计可能不恰好为 100%。
...
升级时先退出统计程序和桌宠，将新版本完整解压到新文件夹，再复制需要保留的 `pet-stats-settings.json`、自定义模型和配置。每日输入历史仍保存在 Windows 用户目录中。发布压缩包排除作者的个人设置、日志、缓存、备份、构建中间文件和测试输出。
...
1. 在 Steam 中安装 Bongo Cat。
2. 从解压后的 Nikki Bongo Cat 文件夹运行 **`CopySteamLaunchOption.cmd`**。
3. 在 Steam 库中右键 **Bongo Cat → 属性 → 通用 → 启动选项**，用复制的内容替换原来的启动选项。
4. 从 Steam 启动 Bongo Cat，或双击 **`StartPetWithStats.Steam.cmd`**。
...
| 问题 | 处理方法 |
| --- | --- |
| 提示找不到 `PetStatsOverlay.exe` | 从 Releases 下载成品 ZIP 并完整解压；源码包不包含编译后的统计程序。 |
...
| Steam 启动了错误的程序 | 重新运行 `CopySteamLaunchOption.cmd`，完整替换 Steam 启动选项。 |
...
最后一条命令会构建自包含程序，并生成 **`artifacts\Nikki-Bongo-Cat-win-x64.zip`**，其中包含桌宠运行时、资源、文档和启动器。推送 `v1.1.0` 等版本标签后，GitHub Actions 会使用同样的打包流程，将 ZIP 和 SHA-256 校验文件附加到 Release。验证工作流也会在推送代码和提交拉取请求时运行自动检查。
