...
- 锁定、解锁时均保持在普通软件和浏览器窗口上方；切换应用时不会抢走输入焦点。
...
Claude 桌面端可能清理缓存，也可能没有缓存尚未在本机打开的会话。程序只能统计实际存在的用量元数据；不会通过计算提示词字数伪造缺失记录。客户端更新也可能改变本地存储格式。缺少可用时间戳的记录无法归入今日，因此不计入今日数据。统计以电脑的本地日期区分“今日”。

**自动更新：**程序每 **5 秒**检查一次用量，面板收起或隐藏时也会继续。Token、预估费用和模型占比一起更新，面板底部的时间显示最近完成的统计，精确到秒。首次读取大量历史记录可能需要更久；未变化的逐行日志会复用读取结果，并定期重新检查。JSONL 长会话超过 128 MB 后仍会继续统计；`MaxLogFileMb` 仍用于限制整份 JSON 文档和 Claude 桌面端数据库文件的大小。

AI 客户端把 Token 用量写入本地日志或缓存后，桌宠才能显示，通常是在一次回复结束后。客户端还未保存的数据无法提前显示。遇到日志只写了一半或读取失败，程序会自动重试，无需靠重启桌宠刷新统计。
...
| Token 数字似乎不动 | 等待 AI 完成本次回复，再等待下一次 5 秒检查完成，并查看面板底部的更新时间。长会话用户请升级到 v1.1.4 或更新版本，旧版会跳过超过 128 MB 的逐行日志。 |
